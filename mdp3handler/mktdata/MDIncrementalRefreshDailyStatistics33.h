...
public:
    ...
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(33);
    ...
    MDIncrementalRefreshDailyStatistics33() = default;
    ...
    MDIncrementalRefreshDailyStatistics33(char *buffer, const std::uint64_t bufferLength) :
        MDIncrementalRefreshDailyStatistics33(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MDIncrementalRefreshDailyStatistics33(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MDIncrementalRefreshDailyStatistics33(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return *this = MDIncrementalRefreshDailyStatistics33(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    MDIncrementalRefreshDailyStatistics33 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        ...
        return *this = MDIncrementalRefreshDailyStatistics33(
            buffer,
    ...
    MDIncrementalRefreshDailyStatistics33 &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = MDIncrementalRefreshDailyStatistics33(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    MDIncrementalRefreshDailyStatistics33 &sbeRewind()
    {
    ...
    {
        MDIncrementalRefreshDailyStatistics33 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
...
#pragma clang diagnostic pop
    ...
    MDIncrementalRefreshDailyStatistics33 &transactTime(const std::uint64_t value) SBE_NOEXCEPT
    {
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, MDIncrementalRefreshDailyStatistics33 _writer)
{
    MDIncrementalRefreshDailyStatistics33 writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "MDIncrementalRefreshDailyStatistics33", )";
