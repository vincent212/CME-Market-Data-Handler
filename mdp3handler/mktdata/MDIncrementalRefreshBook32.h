...
public:
    ...
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(32);
    ...
    MDIncrementalRefreshBook32() = default;
    ...
    MDIncrementalRefreshBook32(char *buffer, const std::uint64_t bufferLength) :
        MDIncrementalRefreshBook32(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MDIncrementalRefreshBook32(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MDIncrementalRefreshBook32(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return *this = MDIncrementalRefreshBook32(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    MDIncrementalRefreshBook32 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        ...
        return *this = MDIncrementalRefreshBook32(
            buffer,
    ...
    MDIncrementalRefreshBook32 &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = MDIncrementalRefreshBook32(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    MDIncrementalRefreshBook32 &sbeRewind()
    {
    ...
    {
        MDIncrementalRefreshBook32 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
...
#pragma clang diagnostic pop
    ...
    MDIncrementalRefreshBook32 &transactTime(const std::uint64_t value) SBE_NOEXCEPT
    {
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, MDIncrementalRefreshBook32 _writer)
{
    MDIncrementalRefreshBook32 writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "MDIncrementalRefreshBook32", )";
