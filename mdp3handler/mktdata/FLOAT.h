...
public:
    ...
    FLOAT(
        ...
        const std::uint64_t bufferLength,
        const std::uint64_t actingVersion) :
        FLOAT(buffer, 0, bufferLength, actingVersion)
    {
    }
    ...
    FLOAT(
        char *buffer,
        const std::uint64_t bufferLength) :
        FLOAT(buffer, 0, bufferLength, sbeSchemaVersion())
    {
    }
    ...
    FLOAT &wrap(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = FLOAT(buffer, offset, bufferLength, actingVersion);
    }
...
#pragma clang diagnostic pop
    ...
    FLOAT &mantissa(const std::int64_t value) SBE_NOEXCEPT
    {
...
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, FLOAT writer)
{
    builder << '{';
    builder << R"("mantissa": )";
