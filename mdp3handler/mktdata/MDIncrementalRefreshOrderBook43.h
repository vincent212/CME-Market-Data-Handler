...
public:
    ...
    static const std::uint16_t SBE_TEMPLATE_ID = static_cast<std::uint16_t>(43);
    ...
    MDIncrementalRefreshOrderBook43() = default;
    ...
    MDIncrementalRefreshOrderBook43(char *buffer, const std::uint64_t bufferLength) :
        MDIncrementalRefreshOrderBook43(buffer, 0, bufferLength, sbeBlockLength(), sbeSchemaVersion())
    {
    }
    ...
    MDIncrementalRefreshOrderBook43(
        ...
        const std::uint64_t actingBlockLength,
        const std::uint64_t actingVersion) :
        MDIncrementalRefreshOrderBook43(buffer, 0, bufferLength, actingBlockLength, actingVersion)
    {
    }
    ...
    {
        return *this = MDIncrementalRefreshOrderBook43(buffer, offset, bufferLength, sbeBlockLength(), sbeSchemaVersion());
    }

    MDIncrementalRefreshOrderBook43 &wrapAndApplyHeader(char *buffer, const std::uint64_t offset, const std::uint64_t bufferLength)
    {
        ...
        return *this = MDIncrementalRefreshOrderBook43(
            buffer,
    ...
    MDIncrementalRefreshOrderBook43 &wrapForDecode(
        ...
        const std::uint64_t bufferLength)
    {
        return *this = MDIncrementalRefreshOrderBook43(buffer, offset, bufferLength, actingBlockLength, actingVersion);
    }

    MDIncrementalRefreshOrderBook43 &sbeRewind()
    {
    ...
    {
        MDIncrementalRefreshOrderBook43 skipper(m_buffer, m_offset, m_bufferLength, sbeBlockLength(), m_actingVersion);
        skipper.skip();
...
#pragma clang diagnostic pop
    ...
    MDIncrementalRefreshOrderBook43 &transactTime(const std::uint64_t value) SBE_NOEXCEPT
    {
...
template<typename CharT, typename Traits>
friend std::basic_ostream<CharT, Traits> & operator << (
    std::basic_ostream<CharT, Traits> &builder, MDIncrementalRefreshOrderBook43 _writer)
{
    MDIncrementalRefreshOrderBook43 writer(
        _writer.m_buffer,
        _writer.m_offset,
    ...
    builder << '{';
    builder << R"("Name": "MDIncrementalRefreshOrderBook43", )";
